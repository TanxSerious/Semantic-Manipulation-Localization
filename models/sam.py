...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None,
        ...
        # ---- 从 encoder_mode 中读取 input_type，传递给 ViT backbone ----
        vit_input_type = encoder_mode.get('input_type', 'wavelet_srm_raw')
        self.image_encoder = build_sam3_image_encoder_only(input_type=vit_input_type)
    ...
    def forward(self, input, gt_mask=None, edge_gt=None):
        # 1. 接收数据
        self.input = input.to(self.device)
        ...
        feats = [feat.permute(1, 2, 0).view(bs, -1, *feat_size)
        ...
        # Predict masks
        low_res_masks, iou_predictions, sam_output_tokens, object_score_logits, edge_pred = self.mask_decoder(
        ...
        if gt_mask is not None:
            ...
            # ---- Edge Loss ----
            edge_loss = torch.tensor(0.0, device=loss_G.device)
            if edge_pred is not None:
                # 使用传入的预计算 edge GT；若未提供则回退到运行时计算
                if edge_gt is not None:
                    edge_gt = edge_gt.to(self.device)
                else:
                    with torch.no_grad():
                        print("Warning: edge_gt not provided, computing edge GT on-the-fly from gt_mask. This may be slow.")
                        gt_for_edge = self.gt_mask  # (B, 1, H, W)
                        pool_kernel = 3
                        pad_size = pool_kernel // 2
                        gt_padded = F.pad(gt_for_edge, [pad_size]*4, mode='reflect')
                        max_pool = F.max_pool2d(gt_padded, kernel_size=pool_kernel, stride=1)
                        min_pool = -F.max_pool2d(-gt_padded, kernel_size=pool_kernel, stride=1)
                        edge_gt = (max_pool - min_pool).clamp(0, 1)  # (B, 1, H, W)

                # Resize edge_pred to match edge_gt
                if edge_pred.shape[-2:] != edge_gt.shape[-2:]:
                    edge_pred_resized = F.interpolate(
                        edge_pred, size=edge_gt.shape[-2:],
                        mode='bilinear', align_corners=False
                    )
                else:
                    edge_pred_resized = edge_pred

                # Balanced BCE for edge (edges are sparse, need pos_weight)
                eps = 1e-10
                pos_count = edge_gt.sum() + eps
                neg_count = (1.0 - edge_gt).sum()
                pos_weight = neg_count / pos_count
                edge_loss = F.binary_cross_entropy_with_logits(
                    edge_pred_resized, edge_gt, pos_weight=pos_weight
                )
                loss_G = loss_G + 0.5 * edge_loss

            self.loss_G = loss_G
            return loss_G, edge_loss
        else:
    ...
    def infer(self, input):
        ...
        low_res_masks, _, _, _, _ = self.mask_decoder(
            image_embeddings=image_embed,
            image_pe=image_pe,
            sparse_prompt_embeddings=sparse_embeddings,
            dense_prompt_embeddings=dense_embeddings,
