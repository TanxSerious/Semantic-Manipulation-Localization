...
class PairedImageFolders(Dataset):
    ...
    def __init__(self, root_path_1, root_path_2, root_path_edge=None, **kwargs):
        self.dataset_1 = ImageFolder(root_path_1, **kwargs)
        self.dataset_2 = ImageFolder(root_path_2, **kwargs, mask=True)
        self.has_edge = root_path_edge is not None
        if self.has_edge:
            self.dataset_edge = ImageFolder(root_path_edge, **kwargs, mask=True)
            assert len(self.dataset_edge) == len(self.dataset_1), \
                f"Edge count ({len(self.dataset_edge)}) != Image count ({len(self.dataset_1)})"
    ...
    def __getitem__(self, idx):
        if self.has_edge:
            return self.dataset_1[idx], self.dataset_2[idx], self.dataset_edge[idx]
